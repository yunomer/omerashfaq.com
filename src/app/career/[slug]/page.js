...
import { getCareerPost, getWritingSeo, getAllCareerPosts } from '@/lib/contentful'
...
async function fetchData(slug) {
  ...
  const data = await getCareerPost(slug, isDevelopment ? true : isEnabled)
  if (!data) notFound()
...
export default async function CareerSlug({ params }) {
  const { slug } = params
  ...
  const jsonLd = {
    ...
    url: `https://omerashfaq.com/career/${slug}`
  }

  return (
    <>
      <ScrollArea className="bg-white" useScrollAreaId>
        <FloatingHeader scrollTitle={title} goBackLink="/career"></FloatingHeader>
...
export async function generateMetadata({ params }) {
  const { slug } = params
  const data = await getCareerPost(slug)
  if (!data) return null
